...
You must output STRICT JSON **with exactly 3 elements with each element containing exactly three top-level string fields**.:
  ...
  - "env": the concrete environment action for THIS step in EXACT format:
        ...
        or      click[<ELEMENT_ID>]
These act as three possible candidates/actions we take on the current page, and we will do MCTS on it. So give 3 alternatives for the same state. 
...
9) The OUTPUT MUST contain 3 possible action candidates, not 1, not 2. Exactly 3
...
6) Avoid searching after you got a list of products, don't go back to search, since you already searched multiple options during MCTS
...
def call_llm(system_prompt: str, user_prompt: str, temperature: float = 0.7) -> str:
    ...
    resp = s.post("http://localhost:8581/v1/chat/completions", data=json.dumps(payload))
...
{"1": {"plan": "string","thought": "string","env": "string},"2": {"plan": "string","thought": "string","env": "string},"3": {"plan": "string","thought": "string","env": "string}} 
