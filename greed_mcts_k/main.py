...
def run_one_goal(goal_idx: int,
                 ...
                 target_high_paths: int = 3,   # stop with ≥3 good paths if no reward1
    ...
    try:
        ...
        # ONE-SHOT GREEDY SEARCH: stop on reward==1 else when ≥ target_high_paths good paths
        best_roll, node_snaps_all, node_snaps_winning, dbg = mcts.search_greedy_full_trajectory(
            goal_idx=goal_idx,
        ...
        # Build a single “training frame” for GRPO / diagnostics
        frames.append({
            "step": 0,
            "instruction": instr,
            "observation": prev_obs,
            "available_actions": prev_avail,
            "history_tuples": list(history_records),   # full composite so far (4-tuples)
            "proposed": {
                "c_puct": c_puct,
                "total_N": None,
                "candidates": []   # node snapshots carry per-node children lists
            },
            "harvested_rollouts": harvested,
            "node_count": dbg.get("node_count"),
            "node_cap": dbg.get("node_cap"),
            "sims_done": dbg.get("sims_done"),
            "paths_found": dbg.get("paths_found"),
            "good_paths": dbg.get("high_paths"),
            "reward1_found": dbg.get("reward1_found"),
            "chosen_policy": "greedy_reward1" if dbg.get("reward1_found") else "greedy_best_reward"
        })

        # If a perfect (or best) terminal rollout exists, DO NOT step live. Use it directly.
        if best_roll is not None:
            final_R = best_roll.reward
            steps = best_roll.length
            history_envs = best_roll.env_path[:]         # env-only path
            history_records = best_roll.actions_full[:]  # full 4-tuples
        ...
        out = {
            ...
            "steps": steps,
            "frames": frames,
            "final_history": history_records,   # composite tuples of the chosen path
            "final_env_path": history_envs,
            "final_node_table": node_table,
            "all_harvested": all_harvested,
            # Listwise CE snapshots:
            # - winning trajectory nodes only
            "node_snapshots_winning": node_snaps_winning,
            # - all expanded nodes during sims (can be used for broader analysis)
            "node_snapshots_all": node_snaps_all
        }
        fname = f"trajs_3/goal_{goal_idx}.json"
...
def run_many(goal_start: int,
             ...
             uct_c_exp: float = 1.0,):
    total, succ = 0, 0
    for g in range(goal_start, goal_end):
        try:
...
if __name__ == "__main__":
    run_many(goal_start=1,
             goal_end=10,
             sims_per_root=20,
             max_steps=12,
             require_reward1=True,
             target_high_paths=2,   # ensure ≥3 good paths if no reward1
             min_reward=0.7,
             path_depth_cap=20,
             max_nodes_per_tree=40,
             uct_c_exp=0.2)
